...
static bool call(ObjFunction *function, int argCount, uint8_t **ip) {
...
static bool callValue(Value callee, int argCount, uint8_t **ip) {
  if (IS_OBJ(callee)) {
    switch (OBJ_TYPE(callee)) {
      case OBJ_FUNCTION: return call(AS_FUNCTION(callee), argCount, ip);
      case OBJ_NATIVE:
  ...
  for (;;) {
    ...
    switch (instruction = READ_BYTE()) {
      ...
      case OP_CALL: {
        ...
        uint8_t *callIp = ip;
        if (!callValue(peek(argCount), argCount, &callIp)) {
          ip = callIp;
          return INTERPRET_RUNTIME_ERROR;
        }
        ip    = callIp;
